...
def apply_lstm(inputs, length, state_size, scope=None, reuse_weights=False):
    scope_name = scope or 'lstm'
    with tf.variable_scope(scope_name, reuse=reuse_weights) as lstm_scope:
        cell = tf.contrib.rnn.core_rnn_cell.BasicLSTMCell(num_units=state_size, state_is_tuple=True)
...
class DualAtt(object):
    def __init__(self,
        ...
        self.h = tf.concat([self.statement_attend, self.side_attend], axis= 1)

        self.scores = feed_forward(self.h, word_embedding_size*2, num_classes, reuse=False)
