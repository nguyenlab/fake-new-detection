...
from memnet.memnet_2side import MemNet, eval
import numpy as np
import tensorflow as tf
import itertools

tf.flags.DEFINE_integer("num_epochs", 60, "number of epochs") # 38 # 57 --> 6 class, 40 --> 2 class
# ch + party stop at 60
# ch + state stop at 52
# ch + job stop at 60
# ch + speaker stop at ... // forgot
# two class, two layer 52
...
tf.flags.DEFINE_integer("num_hops", 1, "the number of hops")
...
# compress all data
train_data = zip(train_x, train_sq_len, train_ch, train_side, train_y)
valid_data = zip(valid_x, valid_sq_len, valid_ch, valid_side, valid_y)
test_data = zip(test_x, test_sq_len, test_ch, test_side, test_y)
...
with graph.as_default():
    with tf.Session() as sess:
        ...
        print("\ntest")
        predictions, attend_weights = eval(test_data, batch_size=FLAGS.batch_size, checkpoint_dir=mem_net.checkpoint_dir, binary=False)
        # print(len(attend_weights))
        # np.savetxt(fname="atention_weights.txt", X=attend_weights)
    # checkpoint_dir = "/home/tinpt/jaist/2-FakeNew-Master/runs/1514306256/checkpoints"
    # predictions, attend_weights = eval(test_data, batch_size=FLAGS.batch_size, checkpoint_dir= checkpoint_dir, binary=False)

    # print("print results")
    # y_test = np.argmax(test_y, axis= 1)
    # for idx, _ in enumerate(y_test):
    #     print("index: {}  truth: {}  pred {}".format(idx + 1, y_test[idx], predictions[idx]))
