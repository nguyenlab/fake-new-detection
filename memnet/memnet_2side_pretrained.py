...
class MemNet(object):
    def __init__(self,
        ...
        self.side_info = tf.placeholder(tf.int32, shape=[None], name="side_info")
        ...
        with tf.name_scope("embedding"):
            ...
            self.W_dep = tf.Variable(initial_value=self.dep_embedding_ph, trainable=False, validate_shape=True,
                                     name="dep_embedding")
            self.word_embeddings.append(self.W_dep)

            self.side_embedding = tf.Variable(tf.truncated_normal(shape=[side_vocab_size, side_size], stddev=0.1),
                                                dtype=tf.float32,
                                                name="tp_embedding")

            self.side_embedded = tf.nn.embedding_lookup(params=self.side_embedding, ids=self.side_info)

        # build the network
        # a version of adding weights

        u_concated = tf.concat([self.input_ch, self.side_embedded], axis= 1)
        concated_size = ch_size + side_size
        u = [u_concated]
        ...
        for hop_i in range(num_hops):
            ...
            p = attend_concate(A, u_k, self.sequence_length, word_embedding_size, concated_size, attention_size, reuse=False,
                               name="attend_{}".format(hop_i))

            o_k = weight_vector(C, p)

            u_k1 = agrregate(u_k, o_k, concated_size, word_embedding_size, concated_size, reuse=False,
    ...
    def train(self, sess, train_data, dev_data, test_data, feed_dict, starter_learning_rate, num_epochs, batch_size, dropout_keep):
        ...
        def train_step(batch):
            x_batch, sq_len_batch, ch_batch, side_batch, y_batch = zip(*batch)
            feed_dict = {
                self.input_x: x_batch,
                self.input_y: y_batch,
                self.sequence_length: sq_len_batch,
                self.side_info: side_batch,
...
def eval(test_data, batch_size, checkpoint_dir, binary=True):
    x_test, sq_length_test, ch_test, side_test, y_test = zip(*test_data)
    print("\nEvaluating...\n")
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            side_info = graph.get_operation_by_name("side_info").outputs[0]
            ...
            batches = data_utils.batch_iter(list(zip(x_test, sq_length_test, ch_test,side_test)), batch_size, shuffle=False)
            # Collect the predictions here
            all_predictions = []

            for batch in batches:
                x_batch, sq_length_batch, ch_batch, side_batch = zip(*batch)
                batch_predictions = sess.run(predictions, {input_x: x_batch,
                                                           sequence_length: sq_length_batch,
                                                           side_info: side_batch,
                                                           ch: ch_batch,
