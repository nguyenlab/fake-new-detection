...
def agrregate2(input1, input2, n1_in, n2_in, n_out, reuse, name="agrregate"):
    """ 
    :param input1: matrix of shape (B, n1) 
    :param input2: matrix of shape (B, n2)
    :param n1_in: 
...
class MemNet(object):
    def __init__(self,
        ...
        for hop_i in range(num_hops):
            ...
            p = attend_concate(A, u_k, self.sequence_length, word_embedding_size, side_size, attention_size, reuse=reuse,
                               name="attend".format(hop_i))
            o_k = weight_vector(C, p)
            self.o.append(o_k)
            u_k1 = agrregate(u_k, o_k, side_size, word_embedding_size, side_size, reuse= reuse,
                             name='agrregate'.format(hop_i))
            u.append(u_k1)

        self.u_drop_out = tf.nn.dropout(u[-1], keep_prob=self.dropout_keep_prob)
        self.scores = feed_forward(self.u_drop_out, side_size, num_classes, reuse=False)
    ...
    def train(self, sess, train_data, dev_data, test_data, starter_learning_rate, num_epochs, batch_size, dropout_keep):
        ...
        def train_step(batch):
            x_batch, sq_len_batch, side_batch, y_batch = zip(*batch)
            feed_dict = {
        ...
        def dev_step(batch, writer=None):
            ...
            step, summaries, loss, accuracy, scores = sess.run(
                [self.global_step, dev_summary_op, self.loss, self.accuracy, self.scores],
                feed_dict)
...
def eval(test_data, batch_size, checkpoint_dir, binary=True):
    x_test, sq_length_test, side_test, y_test = zip(*test_data)
    print("\nEvaluating...\n")
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            batches = data_utils.batch_iter(list(zip(x_test, sq_length_test, side_test)), batch_size, shuffle=False)
            # Collect the predictions here
            all_predictions = []

            for batch in batches:
                x_batch, sq_length_batch, side_batch = zip(*batch)
