...
def add_mask(h, sequence_lenth, max_sequence, pad_value=-np.inf):
    """
    :param h: 2-d vector (?, h_size) 
    :param sequence_lenth: 1-d vector (?)
    :param max_sequence: number
    :param pad_value: 
    :return: 
    """
...
class MemNet(object):
    def __init__(self,
        ...
        self.input_tp = tf.placeholder(tf.int32, shape= [None, tp_len], name= "input_topic")
        self.topic_sq = tf.placeholder(tf.int32, shape= [None], name= "topic_sq")
        ...
        with tf.name_scope("embedding"):
            ...
            self.tp_embedding = tf.Variable(tf.truncated_normal(shape=[tp_vocab_size, tp_size], stddev=0.1), dtype=tf.float32,
                                                name="tp_embedding")
            self.tp_embedded = tf.nn.embedding_lookup(params= self.tp_embedding, ids= self.input_tp)
        ...
        # summary self.tp_embedded
        # (?, 5, 50) --> (?, 50)
        self.tp_avr_embedded = self._get_topic_repr(self.tp_embedded, self.topic_sq, tp_len)
        # build the network
        u = []
        u.append(self.tp_avr_embedded)
        ...
        for hop_i in range(num_hops):
            ...
            p = attend_concate(A, u_k, self.sequence_length, word_embedding_size, tp_size, attention_size, reuse=False,
                               name="attend_{}".format(hop_i))
            o_k = weight_vector(C, p)

            u_k1 = agrregate(u_k, o_k, tp_size, word_embedding_size, tp_size, reuse=False,
                             name='agrregate_{}'.format(hop_i))
            u.append(u_k1)

        self.u_drop_out = tf.nn.dropout(u[-1], keep_prob=self.dropout_keep_prob)
        self.scores = feed_forward(self.u_drop_out, tp_size, num_classes, reuse=False)
    ...
    def _get_topic_repr(self, h, h_sq, max_len):
        """
        :param h: 3-d tensor (?, max_len, h_size), dtype = float32
        :param h_sq: 
        :param max_len: 
        :return: 
        """
        h_size = h.shape[2]
        # (?, 1, max_len) x (?, max_len, h_size) = (?, 1, h_size) = (?, h_size)
        mask2d = tf.sequence_mask(lengths=h_sq, maxlen= max_len)

        ones = tf.ones_like(mask2d, dtype=tf.float32)
        pad_values = 0 * ones

        mask2d = tf.sequence_mask(lengths= h_sq, maxlen= max_len) # act as condition
        mask = tf.where(mask2d, ones, pad_values)
        mask = tf.expand_dims(mask, 1)
        h_masked = tf.matmul(mask, h)
        sum = tf.reshape(h_masked, shape=tf.stack([-1, h_size]))
        h_sq = tf.cast(tf.reshape(h_sq, (-1, 1)), "float")
        avr = tf.divide(sum, h_sq)
    ...
    def train(self, sess, train_data, dev_data, test_data, starter_learning_rate, num_epochs, batch_size, dropout_keep):
        ...
        def train_step(x_batch, sq_len_batch, tp_batch, tp_sq_batch, y_batch):

            feed_dict = {
                self.input_x: x_batch,
                self.input_y: y_batch,
                self.sequence_length: sq_len_batch,
                self.input_tp: tp_batch,
                self.topic_sq: tp_sq_batch,
        ...
        def dev_step(x_batch, sq_len_batch, tp_batch, tp_sq_batch ,y_batch, writer=None):
            feed_dict = {
                self.input_x: x_batch,
                self.sequence_length: sq_len_batch,
                self.input_y: y_batch,
            ...
            print("step %8d, loss %6.4f, acc %6.4f" % (step, loss, accuracy))
            if writer:
        ...
        for i in range(num_epochs):
            ...
            for batch in batches:
                x_batch, sq_len_batch, tp_batch, tp_sq_batch, y_batch = zip(*batch)
                train_step(x_batch, sq_len_batch, tp_batch, tp_sq_batch, y_batch)
            ...
            x_dev, sq_len_dev, tp_dev, tp_sq_dev, y_dev = zip(*dev_data)
            dev_step(x_dev, sq_len_dev, tp_dev, tp_sq_dev, y_dev, writer=dev_summary_writer)

            x_test, sq_len_test, tp_test, tp_sq_test, y_test = zip(*test_data)
            dev_step(x_test, sq_len_test, tp_test, tp_sq_test, y_test, writer=dev_summary_writer)


def eval(test_data, batch_size, checkpoint_dir, binary=True):
    x_test, sq_length_test, tp_test, tp_sq_test, y_test = zip(*test_data)
    print("\nEvaluating...\n")
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            tp = graph.get_operation_by_name("input_topic").outputs[0]
            tp_sq = graph.get_operation_by_name("topic_sq").outputs[0]
            ...
            batches = data_utils.batch_iter(list(zip(x_test, sq_length_test, tp_test, tp_sq_test)), batch_size, shuffle=False)
            # Collect the predictions here
            all_predictions = []

            for batch in batches:
                x_batch, sq_length_batch, tp_batch, tp_sq_batch = zip(*batch)
    ...
    if y_test is not None:
        ...
        print("acc = {:05.4f}, precision= {:05.4f}, recall= {:05.4f}, f1= {:05.4f}".format(acc, precision, recall, f1))
        print("confusion")
        print(confusion)
