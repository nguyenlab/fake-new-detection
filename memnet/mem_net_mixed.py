...
class MemNet(object):
    def __init__(self,
        ...
        concated_size = [ch_size, side_size, side_size]
        u_side = [self.side_embedded1, self.side_embedded2]
        u = [self.input_ch]
        ...
        for hop_i in range(num_hops):
            ...
            p = attend_concate(A, u_k, self.sequence_length, word_embedding_size, concated_size[hop_i], attention_size, reuse=False,
                               name="attend_{}".format(hop_i))

            o_k = weight_vector(C, p)

            u_k1 = agrregate(u_side[hop_i], o_k, concated_size[hop_i + 1], word_embedding_size, concated_size[hop_i +1], reuse=False,
                             name='agrregate_{}'.format(hop_i))
            u.append(u_k1)

        self.u_drop_out = tf.nn.dropout(u[-1], keep_prob=self.dropout_keep_prob)
        # print("test")
        # print(self.u_drop_out.shape)
        # exit()
        self.scores = feed_forward(self.u_drop_out, concated_size[-1], num_classes, reuse=False)
