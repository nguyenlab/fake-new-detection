...
def attend_concate(h, u, sequence_length, H, U, A, reuse):
    """
    ...
    """
    T = h.shape[1]
    with tf.variable_scope("attend", reuse=reuse):  # change to tf.get_variable()
...
class MemNet(object):
    def __init__(self,
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            #self.linear_embedded = tf.nn.embedding_lookup(self.W_linear, self.input_x)
        ...
        for hop_i in range(num_hops):
            ...
            p = attend_concate(A, u_k, self.sequence_length, word_embedding_size, ch_size, attention_size, reuse)
            #o_k = weight_vector(C, p)
            o_k = weight_vector(A, p)
    ...
    def train(self, sess, train_data, dev_data, test_data, starter_learning_rate, num_epochs, batch_size, dropout_keep, feed_dict):
        ...
        def train_step(x_batch, sq_len_batch, ch_batch, y_batch):
            ...
            # h, u_
            # print("\nh")
            # print_list(masked_attend[0][0])
            # print("\nuk")
            # print_list(masked_attend[0][0])
            # print("\nu_")
            # print_list(masked_attend[0][1])
            # print("\nagregate")
            # print_list(masked_attend[0][2])
            # print("\nraw_score")
            # print(masked_attend[0][3])
            # print("\nsequence_length")
            # print(masked_attend[0][4])
            # print("\nmasked_attend")
            # print(masked_attend[0][5])
