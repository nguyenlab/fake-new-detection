...
class MemNet(object):
    def __init__(self,
        ...
        # self.input_tp = tf.placeholder(tf.int32, shape=[None], name="input_topic")
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            self.linear_embedding_ph = tf.placeholder(shape=[vocab_size, word_embedding_size], dtype=tf.float32)
            self.W_linear = tf.Variable(initial_value=self.linear_embedding_ph, trainable= False, validate_shape=True,
                                 name="linear_embedding")

            self.word_embeddings.append(self.W_linear)

            self.dep_embedding_ph = tf.placeholder(shape=[vocab_size, word_embedding_size], dtype=tf.float32)
            self.W_dep = tf.Variable(initial_value=self.dep_embedding_ph, trainable=False, validate_shape=True,
                                 name="dep_embedding")

            self.word_embeddings.append(self.W_dep) # a look up 2 lan

            self.sub_embedding_ph = tf.placeholder(shape=[vocab_size, word_embedding_size], dtype=tf.float32)
            self.W_sub = tf.Variable(initial_value=self.sub_embedding_ph, trainable=False, validate_shape=True,
                                 name="sub_embedding")
            # self.dep_embedded = tf.nn.embedding_lookup(self.W_dep, self.input_x)
            self.word_embeddings.append(self.W_sub)  # a look up 2 lan

        # build the network
        u = []
        u.append(self.input_ch)
        ...
        for hop_i in range(num_hops):
            ...
            A = tf.nn.embedding_lookup(self.word_embeddings[0], self.input_x)  # (?, statement_size, embedding_size)
            C = tf.nn.embedding_lookup(self.word_embeddings[1],
            ...
            p = attend_concate(A, u_k, self.sequence_length, word_embedding_size, ch_size, attention_size, reuse= False, name= "attend_{}".format(hop_i))
            #o_k = weight_vector(C, p)
            o_k = weight_vector(C, p)
            u_k1 = agrregate(u_k, o_k, ch_size, word_embedding_size, ch_size, reuse= False,
                             name='agrregate_{}'.format(hop_i))
            u.append(u_k1)

        self.u_drop_out = tf.nn.dropout(u[-1], keep_prob=self.dropout_keep_prob)
        self.scores = feed_forward(self.u_drop_out, ch_size, num_classes, reuse=False)
    ...
    def train(self, sess, train_data, dev_data, test_data, starter_learning_rate, num_epochs, batch_size, dropout_keep, feed_dict):
        ...
        for i in range(num_epochs):
            ...
            dev_step(x_dev, sq_len_dev, ch_dev, y_dev, writer=dev_summary_writer)

            x_test, sq_len_test, ch_test, y_test = zip(*test_data)
            dev_step(x_test, sq_len_test, ch_test, y_test, writer=dev_summary_writer)
...
def eval(test_data, batch_size, checkpoint_dir, binary=True):
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            batches = data_utils.batch_iter(list(zip(x_test, sq_length_test, ch_test)), batch_size, shuffle=False)
