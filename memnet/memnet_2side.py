...
def attend_concate(h, u, sequence_length, H, U, A, reuse, name):
    ...
    with tf.variable_scope(name, reuse=reuse):  # change to tf.get_variable()
        ...
        p = tf.nn.softmax(masked_attend, name= "attend")
...
def agrregate(input1, input2, n1_in, n2_in, n_out, reuse, name="agrregate"):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        #pre_act = input1 + tf.matmul(input2, W2)
        act = tf.nn.relu(pre_act)
...
class MemNet(object):
    def __init__(self,
        ...
        for hop_i in range(num_hops):
            ...
            #A = add_3dmask(A, self.sequence_length, statment_size, word_embedding_size, mask_value=0)
            #C = add_3dmask(C, self.sequence_length, statment_size, word_embedding_size, mask_value=0)
...
def eval(test_data, batch_size, checkpoint_dir, binary=True):
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            attend_weight = graph.get_operation_by_name("attend_0/attend").outputs[0]
            #attend_0
            ...
            for batch in batches:
                ...
                batch_predictions, attend_weights = sess.run([predictions, attend_weight], {input_x: x_batch,
                ...
                if idx == 0:
                    all_attend_weights = attend_weights
                else:
                    all_attend_weights = np.concatenate([all_attend_weights, attend_weights])
                idx += 1
    ...
    return all_predictions, all_attend_weights