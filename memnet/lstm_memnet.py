...
class MemNet(object):
    def __init__(self,
        ...
        for hop_i in range(num_hops):
            ...
            # put them into a lstm
            A_hidden, _ = self._apply_lstm(A, self.sequence_length, self.hidden_size, scope= "lstm_a_{}".format(hop_i))
            C_hidden, _ = self._apply_lstm(C, self.sequence_length, self.hidden_size, scope= "lstm_c_{}".format(hop_i))
            ...
            p = attend_concate(A_hidden, u_k, self.sequence_length, self.hidden_size, ch_size, attention_size, reuse=False,
                               name="attend_{}".format(hop_i))

            o_k = weight_vector(C_hidden, p)
            u_k1 = agrregate(u_k, o_k, ch_size, self.hidden_size, ch_size, reuse=False,
    ...
    def _apply_lstm(self, inputs, length, state_size, scope=None, dropout_keep_prob= 1, reuse_weights=False):

        scope_name = scope or 'lstm'
        with tf.variable_scope(scope_name, reuse=reuse_weights):
            cell = tf.contrib.rnn.core_rnn_cell.LSTMCell(num_units= state_size, state_is_tuple= True, activation= tf.nn.relu)
            #cell = tf.contrib.rnn.core_rnn_cell.GRUCell(num_units=state_size, activation=tf.nn.elu)
            # cell = tf.contrib.rnn.core_rnn_cell.DropoutWrapper(cell= cell, output_keep_prob= dropout_keep_prob)
